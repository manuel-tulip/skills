...
```mermaid
flowchart TD
  TASK[User task and higher priority instructions] --> MATCH[Skill discovery and task matching]
  MATCH --> CORE[Load relevant SKILL.md]
  CORE --> REF[Load necessary reference sections]
  CORE --> ACT[Execute scoped workflow]
  REF --> ACT
  ACT --> EVID[Record evidence and next state]
  EVID --> RESUME[Resume from compact current state]
```
...
```mermaid
flowchart TD
  START[Load applicable core instructions] --> STATE[Read phase and current checkpoint]
  STATE --> ACTION[Select next unresolved requirement]
  ACTION --> WORK[Implement or investigate]
  ACTION --> CHECK[Validate affected artifacts]
  WORK --> RECORD[Record one milestone]
  CHECK --> RECORD
  RECORD --> PHASE{Ready for final review?}
  PHASE -->|No| STATE
  PHASE -->|Yes| CLOSE[Audit full contract and deliver]
```
